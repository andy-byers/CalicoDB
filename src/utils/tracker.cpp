...
auto Tracker::track(Page &page) -> void
{
    ...
    auto [itr, was_posted] = m_registry.emplace(page.id(), UpdateManager {page.view(0), m_scratch.get()});
...
auto Tracker::discard(Page &page) -> void
{
    ...
    m_scratch.put(itr->second.scratch());
...
auto Tracker::collect(Page &page, SequenceNumber lsn) -> PageUpdate
{
    ...
    if (manager.has_updates()) {
        ...
        auto changes = manager.collect_updates();
