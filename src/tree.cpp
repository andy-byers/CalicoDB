...
auto Tree::make_pivot(const PivotOptions &opt, Cell &pivot_out) -> Status
{
    ...
    pivot_out.footprint = pivot_out.local_size + uint32_t(pivot_out.key - opt.scratch);
...
auto Tree::put(TreeCursor &c, const Slice &key, const Slice &value, bool flag) -> Status
{
    ...
    if (s.is_ok()) {
        ...
        if (s.is_ok() && overflow) {
            ...
            Cell ovfl;
            if (c.m_node.parser(m_cell_scratch[0],
                                m_cell_scratch[1],
                                c.m_node.min_local,
                                c.m_node.max_local,
                                ovfl)) {
...
auto Tree::emplace(Node &node, const Slice &key, const Slice &value, bool flag, uint32_t index, bool &overflow) -> Status
{
    ...
    auto k = key.size();
    auto v = value.size();
    const auto local_size = compute_local_size(k, v, node.min_local, node.max_local);
