...
auto split_record(WalRecordHeader &lhs, const Slice &payload, std::size_t available_size) -> WalRecordHeader
{
    ...
    CDB_EXPECT_LT(available_size, WalRecordHeader::SIZE + payload.size()); // Only call this if we actually need a split.
    auto rhs = lhs;
    ...
    if (lhs.type == WRT_Full) {
        ...
        CDB_EXPECT_EQ(lhs.type, WRT_Last);
        lhs.type = WRT_Middle;
    }
    return rhs;
}
...
static auto decode_deltas_payload(WalPayloadOut in) -> DeltaDescriptor
{
    ...
    // Payload type (1 B)
    CDB_EXPECT_EQ(WalPayloadType {data[0]}, WalPayloadType::WPT_Delta);
    data.advance();
...
static auto decode_full_image_payload(WalPayloadOut in) -> FullImageDescriptor
{
    ...
    // Payload type (1 B)
    CDB_EXPECT_EQ(WalPayloadType {data[0]}, WalPayloadType::WPT_FullImage);
    data.advance();
