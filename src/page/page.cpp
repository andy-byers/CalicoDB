...
Page_::Page_(const Parameters &param)
...
auto Page_::id() const -> Id
{
    return m_id;
}

auto Page_::size() const -> Size
{
    return m_data.size();
}

auto Page_::view(Size offset) const -> Slice
{
    return m_data.range(offset);
}

auto Page_::view(Size offset, Size size) const -> Slice
{
    return m_data.range(offset, size);
}

auto Page_::read(Span out, Size offset) const -> void
{
    mem_copy(out, m_data.range(offset, out.size()));
}

auto Page_::span(Size offset) -> Span
{
    return span(offset, size() - offset);
}

auto Page_::span(Size offset, Size size) -> Span
{
...
auto Page_::write(const Slice &in, Size offset) -> void
{
    mem_copy(span(offset, in.size()), in);
}

auto Page_::apply_update(const FullImageDescriptor &info) -> void
{
...
auto Page_::apply_update(const DeltaDescriptor &info) -> void
{
    CALICO_EXPECT_EQ(m_id, info.pid);
...
auto Page_::collect_deltas() -> std::vector<PageDelta>
{
...
auto get_u16(const Page_ &page, Size offset) -> std::uint16_t
{
    return get_u16(page.view(offset, sizeof(std::uint16_t)));
}

auto get_u32(const Page_ &page, Size offset) -> std::uint32_t
{
    return get_u32(page.view(offset, sizeof(std::uint32_t)));
}

auto get_u64(const Page_ &page, Size offset) -> std::uint64_t
{
