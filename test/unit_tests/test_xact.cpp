...
//            ASSERT_OK(pager->release__(pager->allocate__().value()));
//        }
//
...
//    {
//        auto page = pager->acquire__(id, is_writable);
//        if (!page.has_value()) {
...
//    {
//        auto root = pager->acquire__(Id::root(), true);
...
//
//        return pager->release__(std::move(*root));
//    }
//
//    auto load_state() const -> Status
//    {
//        auto root = pager->acquire__(Id::root(), false);
//        if (!root.has_value()) return root.error();
//
//        auto header = read_header(*root);
...
//
//        auto s = pager->release__(std::move(*root));
