...
public:
    ...
    {
        mem_copy(m_page.span(m_page.size() - VALUE_SIZE, VALUE_SIZE), value);
    }
...
public:
    ...
    auto set_up() -> void
    {
        ...
        while (pager->page_count() < PAGE_COUNT) {
            pager->release(pager->allocate().value());
        }
    ...
    auto load_state() const -> Status
    {
        ...
        const FileHeader header {*root};
        EXPECT_EQ(header.header_crc, header.compute_crc());
    ...
    auto set_value(Id id, const std::string &value) const -> void
    {
        ...
        pager->release(std::move(*wrapper).take());
    }
    ...
    auto get_value(Id id) const -> std::string
    {
        ...
        auto value = wrapper.value().get_value().to_string();
        pager->release(std::move(*wrapper).take());
        return value;
    }
...
static auto insert_records(Test &test, Size n = 1'000)
{
    ...
    for (const auto &r: records) {
        EXPECT_TRUE(expose_message(test.db.put(r.key, r.value)));
    }
    return records;
}
...
{
    for (Size i {}; i < n; ++i) {
        auto cursor = test.db.cursor();
        cursor.seek_first();
        ASSERT_TRUE(cursor.is_valid());
        ASSERT_OK(test.db.erase(cursor.key()));
    }
}
...
static auto run_random_operations(Test &test, const Itr &begin, const Itr &end)
{
    ...
    for (auto itr = begin; itr != end; ++itr) {
        EXPECT_TRUE(expose_message(db.put(itr->key, itr->value)));
    }
    ...
    for (Size i {}; i < limit; ++i) {
        for (const auto &[key, value]: generator.generate(test.random, 100)) {
            ...
            if (test.random.get(4) == 0 && info.record_count()) {
                auto c = test.db.cursor();
                c.seek_first();
                CALICO_TRY_S(c.status());
                s = test.db.erase(c.key());
            } else {
                s = test.db.put(key, value);
            }
            if (!s.is_ok()) return s;
        }
    }
