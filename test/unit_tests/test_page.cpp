...
//    }
//
//    [[nodiscard]] auto free_start() const -> Size
//    {
...
//    std::unordered_map<Size, Size> allocations;
...
{
    auto root = make_node(Id {2}, PageType::EXTERNAL_NODE);
    const auto number = random.get(std::numeric_limits<std::uint64_t>::max() / 2,
...
auto get_maximally_sized_external_cell(NodeTests &test)
{
    ...
    return test.cell_backing.get_cell(test.random.get<std::string>('a', 'z', max_key_length), "value", true, test.arbitrary_pid);
}
...
auto run_maximally_sized_cell_test(NodeTests &test, Id id, Size max_records_before_overflow, bool is_external)
{
    ...
    for (Size i {}; i <= max_records_before_overflow; ++i) {
...
{
    run_maximally_sized_cell_test(*this, Id {Id::root_value + 1}, 5, false);
}
