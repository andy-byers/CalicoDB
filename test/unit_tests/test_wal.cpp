...
class WalIndexTests : public testing::Test
{
protected:
    explicit WalIndexTests()
        : m_index(m_header)
    {
    }

    ~WalIndexTests() override = default;

    auto append(Id page_id)
    {
        ASSERT_OK(m_index.assign(page_id, Id(++m_header.max_frame)));
    }

    WalIndexHeader m_header;
    WalIndex m_index;
};

TEST_F(WalIndexTests, FrameBounds)
{
    append(Id(1));
    append(Id(2));
    append(Id(3));
    ...
    const Id min_frame(2);
    m_header.max_frame = 3;

    Id id;
    ASSERT_OK(m_index.frame_for_page(Id(1), min_frame, id));
    ASSERT_TRUE(id.is_null());
    ASSERT_OK(m_index.frame_for_page(Id(2), min_frame, id));
    ASSERT_EQ(id, Id(2));
    ASSERT_OK(m_index.frame_for_page(Id(3), min_frame, id));
    ASSERT_EQ(id, Id(3));
    ASSERT_OK(m_index.frame_for_page(Id(4), min_frame, id));
    ASSERT_TRUE(id.is_null());
}

TEST_F(WalIndexTests, Cleanup)
{
    Id id;
    append(Id(1));
    append(Id(2));
    ...
    ASSERT_OK(m_index.frame_for_page(Id(1), Id::root(), id));
    ASSERT_EQ(id, Id(1));
    ASSERT_OK(m_index.frame_for_page(Id(2), Id::root(), id));
    ASSERT_EQ(id, Id(2));
    ASSERT_OK(m_index.frame_for_page(Id(3), Id::root(), id));
    ASSERT_EQ(id, Id(3));
    ASSERT_OK(m_index.frame_for_page(Id(4), Id::root(), id));
    ASSERT_EQ(id, Id(4));
...
TEST_F(WalIndexTests, ReadsAndWrites)
{
    std::vector<Id> page_ids;
    // Write 2 full index tables + a few extra entries.
    for (std::size_t i = 0; i < 4'096 * 2; ++i) {
        page_ids.emplace_back(Id(i));
    }
    std::default_random_engine rng(42);
    std::shuffle(begin(page_ids), end(page_ids), rng);

    for (const auto &id : page_ids) {
        append(id);
    }

    const Id min_frame(1'234);
    m_header.max_frame = 5'000;

    auto frame_id = Id::root();
    for (const auto &id : page_ids) {
        ASSERT_EQ(m_index.page_for_frame(frame_id), id);
        Id current;
        ASSERT_OK(m_index.frame_for_page(id, min_frame, current));
        if (m_header.max_frame < frame_id.value || frame_id < min_frame) {
            ASSERT_TRUE(current.is_null());
        } else {
            CHECK_EQ(current, frame_id);
        }
        ++frame_id.value;
    }
}

TEST_F(WalIndexTests, SimulateUsage)
{
    static constexpr std::size_t kNumTestFrames = 100'000;

    tools::RandomGenerator random;
    std::map<Id, Id> simulated;
    ...
    for (std::size_t iteration = 0; iteration < 2; ++iteration) {
        ...
        for (std::size_t frame = 1; frame <= kNumTestFrames; ++frame) {
            if (const auto r = random.Next(10); r == 0) {
                ...
                // will ignore frames below "lower".
                ...
                Id found_id;
                const Id page_id(random.Next(1, kNumTestFrames));
                ASSERT_OK(m_index.frame_for_page(page_id, Id(lower), found_id));
                if (found_id < Id(lower)) {
                    append(page_id);
                    simulated.insert_or_assign(page_id, Id(m_header.max_frame));
                }
            }
        }
        Id found_id;
        for (const auto &[page_id, frame_id] : simulated) {
            ASSERT_OK(m_index.frame_for_page(page_id, Id(lower), found_id));
            CHECK_EQ(found_id, frame_id);
        }
        // Reset the WAL index.
        m_header.max_frame = 0;
        simulated.clear();
...
//     {
//         EXPECT_OK(env->new_file(encode_segment_name(kWalPrefix, id), m_reader_file));
//         return WalReader {*m_reader_file, m_reader_tail};
//     }
//
//     [[nodiscard]] auto make_writer(Id id) -> WalWriter
//     {
//         EXPECT_OK(env->new_log_writer(encode_segment_name(kWalPrefix, id), m_writer_file));
...
//     File *m_reader_file = nullptr;
//     LogWriter *m_writer_file = nullptr;
// };
//
...
//
//     File *editor;
//     ASSERT_OK(env->new_file(encode_segment_name(kWalPrefix, Id::root()), editor));
...
//     LogWriter *logger;
//     ASSERT_OK(env->new_log_writer(encode_segment_name(kWalPrefix, Id::root()), logger));
//     ASSERT_TRUE(cache_first_lsn(*env, kWalPrefix, itr).is_not_found());
//
...
//     {
//         File *temp;
//         EXPECT_OK(env->new_file(encode_segment_name(kWalPrefix, segment_id), temp));
//
//         std::unique_ptr<File> file(temp);
...
// }
//
// TEST(TestWalTable, TestWalTable)
//{
//     WalTable table(Id(1));
//
//     const std::size_t kN = 15;
//
//     for (std::size_t i = 0; i < table.frames(); ++i) {
//         CHECK_OK(table.put(Id(i * 2 + 1), Id(i + 1)));
//     }
//
//     for (std::size_t i = 0; i < table.frames(); ++i) {
//         Id id;
//         CHECK_OK(table.get(Id(i * 2 + 1), Id(table.frames() + 1), id));
//         CHECK_EQ(id, Id(i + 1));
//     }
//
////    CHECK_OK(table.put(Id(1), Id(1)));
////    CHECK_OK(table.put(Id(1 + 383), Id(2)));
////    CHECK_OK(table.put(Id(1 + 383 * 2), Id(3)));
////
////    for (std::size_t i = 0; i < 4096; ++i) {std::cerr << frames[i] << ' ';}std::cerr << '\n';
////    for (std::size_t i = 0; i < 8192; ++i) {std::cerr << hashes[i]<< ' ';}std::cerr << '\n';
////    for (std::size_t i = 0; i < kN; ++i) {
////        Id id;
////        CHECK_OK(table.get(Id(i + 1), Id(kN + 1), id));
////        ASSERT_EQ(id, Id(i + 1));
////    }
//}
