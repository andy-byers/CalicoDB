...
{
    return store.read(to_bytes(buffer));
}
...
{
    return store.write(to_bytes(buffer));
}
...
TEST_F(MemoryTests, SharesMemory)
{
    ...
    ASSERT_EQ(a->write(to_bytes(TEST_STRING)), TEST_STRING.size());
    ASSERT_EQ(b->read(to_bytes(test_buffer)), test_buffer.size());
    ASSERT_EQ(test_buffer, TEST_STRING);
}
...
TEST_F(FaultyMemoryTests, GeneratesWriteFault)
{
    ...
    ASSERT_THROW(mem->write(to_bytes(TEST_STRING)), IOError);
}
