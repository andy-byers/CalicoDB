...
auto main(int argc, const char *argv[]) -> int
{
    ...
    auto db = Database::open(path, options).value();
    {
        ...
        for (Index i {}; i < num_committed; ++i) {
            ...
            CCO_EXPECT_TRUE(db.insert(stob(key), stob(value)).value());
            ofs << value << '\n';
        }
        CCO_EXPECT_TRUE(db.commit().has_value());
    }
    ...
    for (Index i {}; i < LIMIT; ++i) {
        ...
        CCO_EXPECT_TRUE(db.insert(stob(key), stob(value)).has_value());
        ...
        if (const auto info = db.info(); info.record_count() > max_database_size) {
            while (info.record_count() >= max_database_size / 2) {
                ...
                CCO_EXPECT_TRUE(db.erase(cursor.key()).value());
            }
        }
    }
